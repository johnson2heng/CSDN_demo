...
function drawThreeGeo(json, radius, shape, options, limit) {

    var group = new THREE.Group()
    var json_geom = createGeometryArray(json, limit);    
    ...
    for (var geom_num = 0; geom_num < json_geom.length; geom_num++) {
        if (json_geom[geom_num].type == 'Point') {
            ...
            group.add(drawParticle(y_values[0], z_values[0], x_values[0], options));
            ...
            group.add(drawLine(y_values, z_values, x_values, options));
            ...
            for (var segment_num = 0; segment_num < json_geom[geom_num].coordinates.length; segment_num++) {
                ...
                group.add(drawLine(y_values, z_values, x_values));                
            }
...
function createGeometryArray(json, limit) {
    var geometry_array = [];
    ...
    if (json.type == 'Feature') {
        ...
        for (var feature_num = 0; feature_num < json.features.length; feature_num++) {
            if(limit){
                if(json.features[feature_num].id === limit){
